...
def main():
    ...
    if not os.path.exists(calibration_npz_file):
        print(f"FATAL ERROR: Calibration file '{calibration_npz_file}' not found in the current directory.")
        print("Please ensure the calibration file is present where the script is run.")
        return
    try:
        calib_data = np.load(calibration_npz_file)
        mtx_calib, dist_calib = calib_data['mtx'], calib_data['dist']
        print("Calibration data (mtx, dist) loaded successfully.")
