#!/usr/bin/env python
# IMPORTANTA IMPORTANT IMPORTANTA IMPORTANT IMPORTANTA IMPORTANT IMPORTANTA IMPORTANT IMPORTANTA IMPORTANT 
#!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!-------------------------------------------

# instructions:
# pip install svgwrite
# navigate to file folder and run the line below, folder example: DOBOT_MG400_povilas\calibration_capture
# python gen_pattern.py -o chessboard.svg --rows 10 --columns 8 --type checkerboard --square_size 10
...
class PatternMaker:
    def __init__(self, cols, rows, output, units, square_size, radius_rate, page_width, page_height, markers, aruco_marker_size, dict_file):
        ...
        self.width = page_width
        self.height = page_height
        self.dwg = svgwrite.Drawing(output, size=(page_width, page_height), debug=False)  # the svg drawing
    ...
    def make_circles_pattern(self):
        ...
        group = self.dwg.add(self.dwg.g()) # create group
        for x in range(0, self.cols):
            for y in range(0, self.rows):
                dot = self.dwg.circle(center=((x * spacing) + x_spacing + r, (y * spacing) + y_spacing + r), r=r, fill="black", stroke="none")
                group.add(dot)
    ...
    def make_acircles_pattern(self):
        ...
        for x in range(0, self.cols):
            for y in range(0, self.rows):
                dot = self.dwg.circle(center=((2 * x * spacing) + (y % 2)*spacing + x_spacing + r, (y * spacing) + y_spacing + r), r=r, fill="black", stroke="none")
                group.add(dot)
    ...
    def make_checkerboard_pattern(self):
        ...
        for x in range(0, self.cols):
            for y in range(0, self.rows):
                if x % 2 == y % 2:
                    square = self.dwg.rect(insert=(x * spacing + xspacing, y * spacing + yspacing), size=(spacing, spacing), fill="black", stroke="none")
                    group.add(square)
    ...
    def make_radon_checkerboard_pattern(self):
            ...
            for y in range(0, self.rows):
                if x % 2 == y % 2:
                    ...
                    if is_inside:
                        ...
                        square = self.dwg.path(d=self._make_round_rect(x * spacing + xspacing, y * spacing + yspacing,
        ...
        if self.markers is not None:
            ...
            for x, y in self.markers:
                ...
                if x % 2 == y % 2:
                    color = "white"
                dot = self.dwg.circle(center=((x * spacing) + x_spacing + r, (y * spacing) + y_spacing + r), r=r, fill=color, stroke="none")
                group.add(dot)
        ...
        for y in range(0, self.rows):
            for x in range(0, self.cols):
                ...
                if x % 2 == y % 2:
                    ...
                    square = self.dwg.rect(insert=(x_pos+ch_ar_border, y_pos+ch_ar_border), size=(self.aruco_marker_size, self.aruco_marker_size), fill="black", stroke="none")
                    group.add(square)
                    for x_ in range(len(img_mark[0])):
                        for y_ in range(len(img_mark)):
                            if (img_mark[y_][x_] != 0):
                                square = self.dwg.rect(insert=(x_pos+ch_ar_border+(x_)*side, y_pos+ch_ar_border+(y_)*side), size=(side, side), fill="white", stroke="white", stroke_width = spacing*0.01)
                                group.add(square)
